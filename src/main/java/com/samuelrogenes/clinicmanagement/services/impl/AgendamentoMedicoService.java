...
public class AgendamentoMedicoService implements IAgendamentoMedicoService {
	...
	public AgendamentoMedicoDto create(AgendamentoMedicoDto agendamentoDto) {
	    ...
	    validateHorario(agendamentoDto.getDataDaConsulta(), agendamentoDto.getHoraDaConsulta(), null, medico.getId(), paciente.getId());
	...
	public AgendamentoMedicoDto update(Long id, AgendamentoMedicoDto agendamentoDto) {
	    ...
	    validateHorario(agendamentoDto.getDataDaConsulta(), agendamentoDto.getHoraDaConsulta(), id, medico.getId(), paciente.getId());
	...
	private void validateHorario(LocalDate dataDaConsulta, LocalTime horaDaConsulta, Long agendamentoId, Long medicoId,
			Long pacienteId) {
		...
		if (agendamentoId == null) {
			conflito = agendamentoMedicoRepository.findAgendamentosNoHorario(dataDaConsulta, inicioHoraMin, fimHoraMax,
					horaDaConsulta, medicoId, pacienteId).isEmpty();
		} else {
			conflito = agendamentoMedicoRepository.findAgendamentosByHorarioExceptId(dataDaConsulta, inicioHoraMin,
					fimHoraMax, horaDaConsulta, medicoId, pacienteId, agendamentoId).isEmpty();
		}
